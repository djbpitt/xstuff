...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:djb="http://www.obdurodon.org" exclude-result-prefixes="xs" version="3.0"
    ...
    <xsl:variable name="max-loc-str" as="xs:double" select="max(for $loc in $locs return djb:get-text-length($loc))"/>
        ...
        <svg height="{$max_height + 200}" width="{$max_width + 250}">
            <g transform="translate({$half_width + 50}, 100)">
                ...
                <xsl:for-each select="0 to 4">
                    ...
                    <text y="-{$font-size * 1.5}" x="{$pos}" font-size="16" font-family="Times New Roman" text-anchor="middle">
                ...
                <!-- Y Axis label -->
                <text x="{$half_width + $max-loc-str + ($font-size * 2)}" y="{$max_height div 2}" font-family="Times New Roman" text-anchor="middle"
