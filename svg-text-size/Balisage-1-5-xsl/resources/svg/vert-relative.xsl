    ...
    <xsl:template name="xsl:initial-template">
        <svg height="{$max_height + 300}" width="{$max_width + 250}">
            <g transform="translate(100, {$half_height + 125})">
                ...
                <!-- X Axis labels -->
                <text x="{$max_width div 2}" y="{$half_height + $max-loc-str + ($font-size * 3)}" text-anchor="middle"
                    font-size="16">Location at time of writing</text>
                <text x="{$max_width div 2}" y="-{$half_height + ($font-size * 2)}" font-size="16" text-anchor="middle"
                    font-weight="300">
