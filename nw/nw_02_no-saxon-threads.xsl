...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="xsl:initial-template">
        ...
        <xsl:variable name="s1" as="xs:string+" select="tokenize($darwin_1859, '\s+')[position() le 500]"/>
        <xsl:variable name="s2" as="xs:string+" select="tokenize($darwin_1872, '\s+')[position() le 500]"/>
